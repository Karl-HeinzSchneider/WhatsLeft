...
function WL:RescanNow(reason)
    ...
    if not WL._merchantOpen or not MerchantFrame or not MerchantFrame:IsShown() then return end
...
WL:SetScript("OnEvent", function(self, event, ...)
    if event == "ADDON_LOADED" then
        local name = ...
        if name == ADDON then EnsureHooks() end
    ...
    elseif event == "MERCHANT_SHOW" then
        ...
        C_Timer.After(0.15, function()
            if WL._merchantOpen then
                ...
                print(("WL delayed open refresh: %.1f ms (%.1f ms after open)"):format(t1 - t0, t0 - openT))
            end
        end)
...
HKA:SetScript("OnEvent", function(self, event)
    ...
    if event == "MERCHANT_SHOW" or event == "MERCHANT_UPDATE" then
        if self.needsRefresh then self.needsRefresh = false end
...
if type(VKF_TotalsEnabled) ~= "boolean" then VKF_TotalsEnabled = true end
...
local function VKF_ScannerEachLine(fn)
    ...
    for i = 1, (tip:NumLines() or 0) do fn(_G[base .. "TextLeft" .. i], _G[base .. "TextRight" .. i], i) end
end
...
local function TooltipHasCollected(slot)
    ...
    for i = 1, scanner:NumLines() do
        ...
        local a1, b1 = extract(L);
        if a1 and a1 > 0 then return true end
        local a2, b2 = extract(R);
        if a2 and a2 > 0 then return true end
    end
    return false
end
...
local function TooltipContainsAnyWord(slot, tokenList)
    ...
    for _, token in ipairs(tokenList) do if TooltipContainsWordLower(slot, token) then return true end end
    return false
end
...
local function ItemTeachesPet(slot, link)
    ...
    if VKF_SafeScannerSetMerchantItem(slot) then
        ...
        if TooltipContainsAnyWord(slot, LOC.PET_TOKENS) or
            (TooltipContainsAnyWord(slot, LOC.TEACHES_TOKENS) and TooltipContainsAnyWord(slot, LOC.SUMMON_TOKENS)) then
            return true
        end
    end
...
local function IsPetKnown(slot, link)
    ...
    if type(speciesID) == "number" and C_PetJournal and C_PetJournal.GetNumCollectedInfo then
...
local function TooltipContainsAnyToken(slot, tokenList)
    if not tokenList then return false end
    for _, token in ipairs(tokenList) do if TooltipContainsLower(slot, token) then return true end end
    return false
end
...
local function TooltipHasCollected(slot)
    ...
    VKF_ScannerEachLine(function(L, R)
        ...
        local a = select(1, extract(L));
        if a > 0 then ok = true end
        local b = select(1, extract(R));
        if b > 0 then ok = true end
    end)
    return ok
end
...
local function TooltipIsTemporarilyUnavailable(slot)
    if not VKF_IsValidSlot(slot) then return false end
    for _, p in ipairs(LOC.UNAVAILABLE_PATTERNS or {}) do if TooltipContainsLower(slot, p) then return true end end
    return false
end
----------------------------------------------------------------
-- CATEGORY CHECKS
...
local function IsEnsemble(link, slot)
    ...
    for _, pattern in ipairs(LOC.ENSEMBLE_NAME_TOKENS or {}) do if name:find(pattern, 1, true) then return true end end
    -- Fallback for any future wording: detect via tooltip text
    if slot and TooltipContainsAnyToken(slot, LOC.ENSEMBLE_TIP_TOKENS) then return true end
    return false
end
...
local function IsToyKnown(slot, link)
    ...
    if toyItemID and PlayerHasToy and PlayerHasToy(toyItemID) then return true end
...
local function DetectMount(slot, link)
    ...
    if slot and TooltipContainsAnyToken(slot, LOC.MOUNT_TOKENS) and not TooltipContainsAnyToken(slot, LOC.TOY_TOKENS) and
        not TooltipContainsAnyToken(slot, LOC.APPEARANCE_TOKENS) then return true end
...
local function DetectPet(slot, link)
    ...
    if link:find("|Hbattlepet:%d+:") then return true end

    -- Tooltip fallback: only match clear pet phrases (avoid catching mounts)
    if slot then if TooltipContainsAnyToken(slot, LOC.PET_TOKENS) then return true end end
...
local function DetectToy(slot, link)
    ...
    if slot and TooltipContainsAnyToken(slot, LOC.TOY_TOKENS) then return true end
    return false
end
...
local function GetTooltipTextLower(slot, link)
    ...
    if cached then return cached.lower end
    ...
    for i = 1, TT:NumLines() do
        ...
        if line then
            ...
            if t and t ~= "" then text[#text + 1] = t end
        end
    end
...
local function VKF_GetSlotMeta(slot)
    if VKF_SlotMeta[slot] ~= nil then return VKF_SlotMeta[slot] end
...
local function BuildFilteredSlots()
    ...
    for slot = 1, total do
        ...
        if not link then
            ...
            ---------------------------------------------------------
            if not hide then table.insert(out, slot) end
        end
    end
    return out
end
...
-- ROW PAINT
----------------------------------------------------------------
local function RowNameFS(i, row)
    return (row and row.Name) or _G["MerchantItem" .. i .. "Name"]
end
local function RowIconTX(i, btn)
    return (btn and (btn.icon or btn.Icon)) or _G["MerchantItem" .. i .. "ItemButtonIconTexture"]
end
...
local function PaintRow(i, slot)
    ...
    if slot then
        ...
        if MerchantFrameItem_Update then MerchantFrameItem_Update(row, slot) end
...
if type(VKF_ToggleMini) ~= "function" then
    ...
    local function VKF_FillMiniWithBreakdown(f)
        ...
        -- Layout & fill
        f.line1:ClearAllPoints();
        f.line1:SetPoint("TOPLEFT", f.title, "BOTTOMLEFT", 0, -8)
        local used = #rows

        if used >= 1 then
            f.line1:SetText(rows[1]);
            f.line1:Show()
        else
            f.line1:Hide()
        end

        f.line2:ClearAllPoints();
        f.line2:SetPoint("TOPLEFT", f.line1, "BOTTOMLEFT", 0, -6)
        if used >= 2 then
            f.line2:SetText(rows[2]);
            f.line2:Show()
        else
            f.line2:Hide()
        end

        f.line3:ClearAllPoints();
        f.line3:SetPoint("TOPLEFT", (f.line2:IsShown() and f.line2 or f.line1), "BOTTOMLEFT", 0, -6)
        if used >= 3 then
            f.line3:SetText(rows[3]);
            f.line3:Show()
        else
            f.line3:Hide()
        end
    ...
    function VKF_EnsureMiniPanel(anchorFrame)
        ...
        function VKF_HideMini()
            if _G.VKF_MiniPanel and _G.VKF_MiniPanel.Hide then _G.VKF_MiniPanel:Hide() end
        end
        _G.VKF_HideMini = VKF_HideMini
...
local function EnsureTotalsPanel()
    ...
    VKF_TotalsPanel:SetScript("OnSizeChanged", function(_, _, h)
        VKF_TotalsState.h = h
    end)
...
local function AnchorTotalsToMenu()
    ...
    if VKF_TotalsPanel:GetParent() ~= _G.VKF_MenuPanel then VKF_TotalsPanel:SetParent(_G.VKF_MenuPanel) end
...
local function AddLine(panel, iconFile, label, amount, yOff)
    ...
    row.icon:SetPoint("TOPLEFT", 6, yOff);
...
local function BuildTotals()
    ...
    for slot = 1, totalSlots do
        ...
        if CountThis() then
            ...
            if price and price > 0 then totals.money = totals.money + price end
                    ...
                    if curID then
                        ...
                        if itemID then
                            ...
                            local entry = totals.itm[key] or
                                              {name = name or ("Item " .. itemID), icon = icon or tex, amount = 0}
                            entry.amount = entry.amount + amount
...
local function BuildCategoryBreakdown()
    local data = {pets = {total = 0, known = 0}, toys = {total = 0, known = 0}, countTotal = 0, countKnown = 0}
    ...
    do
        ...
        if b then
            ...
            if pick and pick.amount and pick.amount > 0 then
                ...
                local amtText = BreakUpLargeNumbers and BreakUpLargeNumbers(pick.amount) or tostring(pick.amount)
                local label = pick.name or
                                  (LOC and LOC.BRONZE_TOKENS and LOC.BRONZE_TOKENS[1] and
                                      LOC.BRONZE_TOKENS[1]:gsub("^(.)", string.upper) or "Bronze")
                b.text:SetText(string.format("%s: |cffffffff%s|r", label, amtText))
...
function HKA:Refresh()
    ...
    pcall(function()
        ...
        local str = string.format("|cffffd100%sBench: |r|cff8080ff%s|r took |cffffd100%.4f|r ms", '0',
                                  'HKA:Refresh() > BuildFilteredSlots()', duration * 1000)
        print(str)
        ------------------------------------------------------
        ...
        ------------------------------------------------------
        for i = 1, per do PaintRow(i, filtered[first + i - 1]) end
...
function HKA:RepaintOnly()
    ...
    pcall(function()
        ...
        print(("VKF repaint: total=%d, filtered=%d, page=%d, showing=%d"):format(total, #filtered, page, shown))
        ...
        if pageText and pageText.SetText then
            ...
            C_Timer.After(0.8, function()
                if pageText and old then pageText:SetText(old) end
            end)
        end
...
local GEAR_ATLASES = {
    "commonoptions-gear", "OptionsIcon-Brown", "OptionsIcon-White", "Settings_Gear", "commonoptions-gear-64x64"
}
...
local function EnsureMenuButton()
    ...
    VKF_MenuButton:SetScript("OnLeave", function()
        GameTooltip:Hide()
    end)
end

local function VKF_FindFilterDropdown()
    local cand = _G.MerchantFrameFilterDropdown or
                     (MerchantFrame and
                         (MerchantFrame.filterDropdown or MerchantFrame.FilterDropdown or MerchantFrame.FilterDropDown or
                             MerchantFrame.LootFilterDropdown))
    ...
    local function score(f)
        ...
        for _, r in ipairs {f:GetRegions()} do
            if r and r.GetObjectType and r:GetObjectType() == "FontString" and r:GetText() and r:GetText() ~= "" then
                s = s + 2;
                break
            end
        end
...
function EnsureMenuPanel()
    ...
    VKF_MenuPanelState = S;
    _G.VKF_MenuPanelState = S
    ...
    panel:SetScript("OnMouseDown", function(self, btn)
        if btn == "LeftButton" and not S.locked then
            self._moving = true;
            self:StartMoving()
        end
    end)
    ...
    title:SetPoint("TOPRIGHT", -24, -6);
    title:SetHeight(22)
    local titleText = panel:CreateFontString(nil, "OVERLAY", "GameFontHighlightLarge")
    titleText:SetPoint("LEFT", title, "LEFT", 4, -1);
    titleText:SetText(LOC.UI_APP_TITLE)
    ...
    sizer:SetScript("OnMouseUp", function(self)
        if self._resizing then
            ...
            if dw <= 2 and dh <= 2 then
                panel:SetSize(self._startW, self._startH);
    ...
    function panel:ApplyLock()
        ...
        if not self._lockText then self._lockText = self:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall") end
    ...
    local function makeCheck(y, label, key, opts)
        ...
        if opts.disabled then
            cb:Disable();
            cb:SetAlpha(0.7);
            cb.text:SetTextColor(0.7, 0.7, 0.7)
            cb:SetScript("OnEnter", function(self)
                GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
                GameTooltip:AddLine(LOC.UI_COMING_SOON, 1, 1, 1);
                GameTooltip:Show()
            end)
    ...
    local y = TOP_Y - 22
    makeCheck(y, LOC.UI_LABEL_SETS, "VKF_HideSets");
    y = y - ROW_H
    makeCheck(y, LOC.UI_LABEL_MOUNTS, "VKF_HideMounts");
    y = y - ROW_H
    makeCheck(y, LOC.UI_LABEL_PETS, "VKF_HidePets");
    y = y - ROW_H
    makeCheck(y, LOC.UI_LABEL_TOYS, "VKF_HideToys");
    y = y - ROW_H
    ...
    makeCheck(y, LOC.UI_LABEL_SETS, "VKF_SkipUnavail_Sets");
    y = y - ROW_H
    makeCheck(y, LOC.UI_LABEL_MOUNTS, "VKF_SkipUnavail_Mounts");
    y = y - ROW_H
    makeCheck(y, LOC.UI_LABEL_PETS, "VKF_SkipUnavail_Pets");
    y = y - ROW_H
...
local function BuildMenuUI()
    ...
    AnchorTotalsToMenu()
    UpdateTotalsUI()
    if _vkf_firstOpen then
        _vkf_firstOpen = false;
        _G.VKF_MenuPanel:Show()
    end
end
...
local function VKF__FindIconRegion(dialog)
    ...
    if n then
        local g = _G[n .. "ItemFrameIconTexture"] or _G[n .. "ItemFrameIcon"] or _G[n .. "IconTexture"] or
                      _G[n .. "AlertIcon"]
    ...
    for i = 1, num do
        ...
        if r and r.IsObjectType and r:IsObjectType("Texture") and r.SetTexture then return r end
    end
    -- scan shallow children
    for _, child in ipairs({dialog:GetChildren()}) do
        if child and child.IsObjectType and child:IsObjectType("Texture") and child.SetTexture then return child end
...
local function VKF__SlotFromDialog(dialog)
    ...
    if type(d) == "table" then return d.index or d.slot or d.merchantSlot or d.slotIndex or d.itemIndex end
end
...
local function VKF__TickPopups()
    ...
    if not any then VKF_PopupWatcher:SetScript("OnUpdate", nil) end
end
    ...
    for i = 1, N do
        ...
        if d and not d.VKF_Watched then
            ...
            d:HookScript("OnHide", function(self)
                ...
                if not next(activeDialogs) then VKF_PopupWatcher:SetScript("OnUpdate", nil) end
            end)
        end
    end
end
...
local function VKF_RecolorAltCurrencyRow_Scan(rowIndex)
    local row = _G["MerchantItem" .. rowIndex];
    if not row then return end
    local btn = row.ItemButton or _G["MerchantItem" .. rowIndex .. "ItemButton"];
    if not btn then return end
    local slot = btn:GetID();
    if type(slot) ~= "number" or slot <= 0 then return end

    local acf = _G["MerchantItem" .. rowIndex .. "AltCurrencyFrame"];
...
local VKF_CHANGELOG = {
    {
        ...
        notes = {
            ...
            "Minor fixes to breakdown totals and progress display alignment.", "— — — — —", "Upcoming:",
    ...
    }, {version = "1.0.0", date = "2025-10-24", notes = {"Initial public release for Remix vendors."}}
}
...
local function VKF_BuildChangelogText()
    ...
    for _, entry in ipairs(VKF_CHANGELOG) do
        ...
        for _, line in ipairs(entry.notes or {}) do table.insert(t, "  • " .. line) end
...
SlashCmdList["WL_CHANGELOG"] = function()
    VKF_ShowChangelog()
end
